  ...
  const sponsorList = sponsors.map(s => {
    ...
    return <>
        ...
        marginBottom: 40
          ...
          {group.map(sponsor => <a class="text-center" target="_blank" href={"".concat(sponsor.link || '#')}>
              <_StyledImg src={"/images/sponsors/".concat(sponsor.image)} class="img-sponsor-div2 img-first-div animate__animated animate__zoomIn" $_css={sponsor.css || ''} />
            </a>)}
        </div>
      </>;
  });
  return <section id="sponsors" class="triangle-bg-2 jumbotron jumbotron-fluid align-content-center" style={{
    marginBottom: 0
  }}>
      <div class="container">
